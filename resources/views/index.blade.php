@extends('statamic::layout')

@section('title', __('Acumbamail'))

@section('content')

    @unless($formConfigs->isEmpty())

        <acumbamail-listing
            create-form-url="{{ cp_route('forms.create') }}"
            configure-url="{{ $configureUrl }}"
            :initial-form-configs="{{ json_encode($formConfigs) }}"
            :initial-localizations="{{ empty($localizations) ? '[]' : json_encode($localizations) }}"
            initial-site="{{ empty($locale) ? '' : $locale }}"
        ></acumbamail-listing>

    @else

        @include('statamic::partials.empty-state', [
            'title' => __('Acumbamail'),
            'description' => 'Forms are used to collect information from visitors and synchronize the data with Acumbamail when there is a new submission.',
            'svg' => 'empty/form',
            'button_text' => __('Create Form'),
            'button_url' => cp_route('forms.create'),
        ])

    @endunless

@endsection
